...
SERVICOS_CHOICES = (('SERVIÇOS', 'SERVIÇOS',), ('Consultoria', 'Consultoria',), ('Capacitação', 'Capacitação',), ('Maximização de Lucros', 'Maximização de Lucros'), ('Outros', 'Outros',))
class Contato(forms.Form):
	...
	servico = forms.ChoiceField(label= 'Serviço', widget=forms.Select() , choices=SERVICOS_CHOICES)
	...
	def send_mail(self):
		subject = self.cleaned_data['servico']
		message = ' Nome: %(nome)s\n E-mail: %(email)s\n Telefone: %(telefone)s\n Serviço: %(servico)s \n \n %(message)s '
		context = {
